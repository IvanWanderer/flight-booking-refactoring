...
const db = new FlightDatabase(flightsData);  // Теперь видит глобальный flightsData
...
const updateResults = () => {
  ...
  renderer.render(results, (flight, button) => {  // Передаём button из рендера
    if (cart.add(flight)) {
...
document.getElementById("book-now").addEventListener("click", () => {
  ...
  cart.clear();
  updateResults(); // сбрасываем кнопки
});
...
// Первый запуск
updateResults();
console.log("main.js loaded, initial render called");  // Для отладки